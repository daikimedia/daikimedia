...
const ServiceBoxes = () => {
  ...
  return (
    <motion.div
      ...
      {ServiceData?.filter((items) => items.id >= 1 && items.id <= 6).map((items) => (
        <div
          ...
          <div className="h-full rounded border border-dashed border-gray-100 dark:text-white p-10 dark:border-borderColor-dark max-lg:p-5">
            ...
            <Link href={`/services/${items.slug}`} className="btn-outline btn-sm dark:bg-white dark:text-black hover:dark:bg-black hover:dark:text-white">
              Read More
            </Link>
          </div>
        </div>
